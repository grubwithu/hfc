...
inline DotGraph *DotParser::ParseFromString(const std::string &_sGraph, const bool _bApplyGlobalAttributesToNodes) {
  DotGraph *pGraph = new DotGraph();
  DotGraph &graph = *pGraph;
  ...
  size_t uGraphDesc = hlx::find_first_of(_sGraph, GraphTypes, uGraphType);
  if (uGraphDesc != std::string::npos) {
  ...
  for (const std::string &sStatement : Statements) {
    ...
    // std::cout << sStatementBody << std::endl;
    ...
    if (EdgesNodes.size() <= 1u) {
      ...
      if (sNodeName.empty() == false) {
        if (IsGlobalAttrib(sNodeName)) {
          ...
          auto node = graph.AddNode(sNodeName);
          node->AddAttributes(Attributes);
        }
      }
    } else {
      std::string sNodeName = hlx::trim(EdgesNodes.front());
      auto idx = sNodeName.find(':');
      if (idx != sNodeName.npos) {
        sNodeName = sNodeName.substr(0, idx);
      }
      if (sNodeName.empty() == false) {
...
inline TAttributes DotParser::ParseAttributes(const std::string &_sAttribList) {
  ...
  for (size_t idx = 0, lastEscape = 0, quoteStart = -1; idx < sBody.size(); ++idx) {
    if (lastEscape) {
      lastEscape = 0;
      continue;
    }
    ...
    if (sBody[idx] == '\"') {
      if (quoteStart == -1) {
        ...
        sQuotes.emplace_back(sBody.substr(quoteStart + 1u, idx - quoteStart - 1u));
        sBody.replace(quoteStart, idx - quoteStart + 1u, sToken);
        idx = quoteStart + sToken.length() - 1;
        quoteStart = -1;
      }
    }
  }
