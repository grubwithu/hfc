...
std::vector<std::string> ParseFuzzers(const char *fuzzers) {
  ...
  if (Flags.fuzzers[0] == '@')
    ...
    Fuzzers = Flags.fuzzers; // fuzzers contains the list.
  if (Fuzzers.empty()) {
    Printf("fuzzers is empty or @file does not exist.\n");
    exit(1);
  }
  // Parse Fuzzers.
  size_t comma_pos = 0;
  while ((comma_pos = Fuzzers.find_last_of(',')) != std::string::npos) {
    FuzzerList.push_back(Fuzzers.substr(comma_pos + 1));
    Fuzzers = Fuzzers.substr(0, comma_pos);
  }
  FuzzerList.push_back(Fuzzers);
  return FuzzerList;
}
